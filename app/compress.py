...
def compress_pdf_safe(input_path: str, output_path: str, target_size: int) -> dict:
    ...
    if original_size <= target_size:
        ...
        return {'original_size': original_size, 'compressed_size': original_size, 'reduction': '0%'}

    pdf = pdfium.PdfDocument(input_path)
    n_pages = len(pdf)
    
    # Heuristic: Calculate target size per page
    target_per_page = target_size // max(1, n_pages)
    
    # 1. Rasterize
    images = []
    scale = 2 # Good quality default
    ...
    for s in scales:
        for q in qualities:
            ...
            try:
                ...
                buffer = io.BytesIO()
                temp_images[0].save(
                    buffer, "PDF", resolution=72 * s, 
                    save_all=True, append_images=temp_images[1:], 
                ...
                if best_result is None or size < best_result['size']:
                    best_result = {'size': size, 'data': buffer.getvalue(), 'desc': f'Scale {s}x, Q{q}'}
    ...
    # Failed completely
    shutil.copy(input_path, output_path)
    return {'original_size': original_size, 'compressed_size': original_size, 'info': 'Failed to compress'}
...
def compress_file(input_path: str, output_path: str, target_size: int) -> dict:
    ...
    if in_p.suffix.lower() == '.pdf' and out_p.suffix.lower() == '.pdf':
        ...
        return compress_image_to_size(output_path, output_path, target_size) # compress in place
