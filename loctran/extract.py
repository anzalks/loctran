...
try:
    ...
    def detect_langs(_text):
        raise LangDetectException("langdetect is not installed. Install with: pip install loctran")
...
def _get_pillow_image():
    try:
        ...
        raise RuntimeError("Missing optional dependency 'Pillow'. Install with: pip install loctran") from exc
    return Image
...
def _clean_ocr_response(text):
    ...
    skip_phrases = [
        ...
        'No markdown', 'Extract the text', 'Read the text',
        'markdown', '```',
    ]
    for line in lines:
    ...
    try:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            ...
            for w in words:
                 segments.append({
                     "text": w['text'],
                     "bbox": [w['x0'], w['top'], w['x1'] - w['x0'], w['bottom'] - w['top']], # x, y, w, h
                     "method": "Digital"
                 })
    except Exception:
        pass
        ...
        for w in all_words:
            ...
            for existing in unique_words:
                ...
                if w['text'] == existing['text']:
                    ...
                    x2 = min(w['left']+w['width'], existing['left']+existing['width'])
                    y2 = min(w['top']+w['height'], existing['top']+existing['height'])
        ...
        for line in lines:
            ...
            for i in range(1, len(line)):
                ...
                if gap > gap_threshold:
                    ...
                    process_individual_segment(current_segment_words, segments, image_path, use_ai, img, vision_model)
...
def process_individual_segment(word_list, segments, image_path, use_ai, img_obj, vision_model="glm-ocr:latest"):
    ...
    if use_ai and (bbox[2] > 20) and (bbox[3] > 10):
        try:
             ...
             if os.getenv("LOCTRAN_DEBUG"): print(f"[WARN] AI Segment failed: {e}")
             ...
             try:
                 ...
                 with pdfplumber.open(pdf_path_str) as pdf:
                     ...
                     if text and len(text.strip()) > 50:
                         ...
                         med_h = sorted([w['height'] for w in words])[len(words)//2] if words else 12
                         line_gap = med_h * 0.6
                         ...
                         for line in lines:
                             ...
                             char_widths = [w['x1'] - w['x0'] for w in line if len(w['text']) > 0]
                             if char_widths:
                                 med_char_w = sorted(char_widths)[len(char_widths)//2] / max(1, len(line[0]['text']))
                             else:
                             ...
                             for i in range(1, len(line)):
                                 ...
                                 if gap > gap_threshold:
                                     segments.append(merge_words(current_chunk, scale_x, scale_y))
                                     current_chunk = []
                 ...
                 if os.getenv("LOCTRAN_DEBUG"): print(f"[WARN] Digital extraction failed: {e}")
        
        if not got_digital:
            # 2. Hybrid OCR
            segments = get_segments_hybrid(image_path, use_ai=use_ai_ocr, vision_model=vision_model)
            item['segments'] = segments
...
def _process_text_file(txt_path, doc_dir, progress_callback=None):
    ...
    if len(paragraphs) == 1 and '\n' in paragraphs[0]:
        lines = [l.strip() for l in paragraphs[0].splitlines() if l.strip()]
...
def process_file(pdf_path, output_dir, progress_callback=None, folder_name=None, use_ai_ocr=False, force_ocr=False, vision_model="glm-ocr:latest"):
    if folder_name:
    ...
    if pdf_path.suffix.lower() in ['.jpg', '.jpeg', '.png']:
        ...
        if progress_callback: progress_callback(msg, 10)
        else: print(msg)
        ...
        try:
            ...
            if progress_callback: progress_callback(err, 0)
            else: print(err)
            return
    ...
    if progress_callback: progress_callback(msg, 20)
    
    tasks = [(str(pdf_path), p, i, use_ai_ocr, force_ocr, vision_model) for i, p in enumerate(image_paths)]
    ...
    if progress_callback: progress_callback(msg, 85)
...
def main():
    ...
    parser.add_argument("--use-ai-ocr", action="store_true", help="Use Ollama (DeepSeek) for OCR segments (slower but better for tables/handwriting)")
    ...
    for f in files:
        process_file(f, output_dir, force_ocr=args.force_ocr, use_ai_ocr=args.use_ai_ocr)
